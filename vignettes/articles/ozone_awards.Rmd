---
title: "Most Ozone-Polluted City in Mexico"
...
df_filtered <- o3_2017 %>%
  ...
  # filter values above .155 (phase I pollution contingency)
  # that occurred during the night, since ozone is a photosensitive
  # pollutant
  filter(!(value > .155 & hour %in% c(0:12, 20:23))) %>%
  filter(value < .3)
...
df_max <- df_max %>%
  ...
  time_decompose(max, method = "twitter", trend = "3 months") %>%
