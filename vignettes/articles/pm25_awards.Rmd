...
It was decided to remove Irapuato from the rankings and keep Tepic because PM<sub>2.5</sub> values can spike because of fires. Normally I would have removed Mexicali since it didn't report any correct data during the winter season, but its levels of PM<sub>2.5</sbu> where so high that I decided to keep it.
...
plot_ly(as.data.frame(top3), x = ~date, y = ~`Toluca`, name = "Toluca" ,
  ...
  add_trace(y = ~Cuernavaca, name = 'Cuernavaca', mode = 'lines', line = list(color = '#377eb8'), width = .5) %>%
