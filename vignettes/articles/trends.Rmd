...
guadalajara <- get_data("Guadalajara", "O3", remove_extremes = FALSE) %>%
  ...
  ggtitle("Daily ozone values reported by allGuadalajara stations") +
  xlab("date") +
  ylab("ppm") +
  theme_bw()
```
...
```{r toluca, cache=TRUE}
get_data("Toluca", "PM2.5", remove_extremes = FALSE) %>%
  ggplot(aes(as.Date(date), value)) +
...
get_data("Pachuca", "PM2.5", remove_extremes = FALSE) %>%
  ggplot(aes(as.Date(date), value)) +
  geom_point(alpha = .3, size = .3) +
  geom_smooth(method = 'gam', formula = y ~ s(x)) + 
  ggtitle(expression(paste("Daily ", PM[2.5], 
...
get_data("Monterrey", "PM10", remove_extremes = FALSE) %>%
  ggplot(aes(as.Date(date), value)) +
  geom_point(alpha = .2, size = .3) +
  geom_smooth(method = 'gam', formula = y ~ s(x)) + 
